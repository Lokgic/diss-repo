...
be understood without taking into account the \emph{context} of inquiry in which it is made. An
...
\tightlist
\item
  Starting with \ref{fig:freqhistsuc}, perhaps the first useful comparison is one between the number of successes between trials with a fixed $n=2000$  number of attempts versus trials with optional stopping allowed. In accordance with the Central limit theorem, the left hand side forms a nice normal distribution.\footnote{\cite{degroot}, section 6.3} The right hand side with optional stopping forms a bimodal distribution.
  
  \item As seen in \ref{fig:freqfixedvsoptional}, 371 out 1000 outcomes has reached significance, i.e., (\(\text{p value}\leq0.05\)). P values for trials with fixed N(left figure) are almost uniformly distributed.
\item
...
Bayesian perspective, it suggests both a criticism of frequentism and support for Bayesian statistics. The criticism is that the
...
\emph{counterfactual probabilities}. The support is that Bayesian
...
to get the probabilities needed, we will have to decide on the probability of a Hit: if we accept \emph{provisionally} that a subject is just making randomly guesses, then the probability of a Hit is $0.2$, and the probability of a Miss is $0.8$. With these assumptions in place, we can calculate:
...
Bayesians, such as Lindley, criticizes the use of probabilities of counterfactual scenarios in the evaluation of statistical hypotheses:
...
Ramsey, and then later, independently, I. J. Good, and L. J. Savage.\footnote{\cite{ramseyvalue}}\footnote{\cite{goodtotalevidence}}\footnote{\cite{savage}, sec 6.2.}

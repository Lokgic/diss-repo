...
\tightlist
\item
  371 out 1000 outcomes has reached significance. (\(\text{p value}\leq0.05\)) In figure \ref{fig:freqhistp}, we see that the number of outcomes with significant p values far outnumbers the rest.
\item
...
\item
  ...
  is not always the case. This makes intuitive sense, since we know that the larger the sample size is, the closer the result will approximate the true proportion. Amazingly, one test reached significance at the 1999th attempt.
\end{enumerate}
...
is simply:\footnote{\cite{degroot}}
